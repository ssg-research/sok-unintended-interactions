...
def main(args: argparse.Namespace, log: logging.Logger) -> None:
    ...
    if args.dataset == "CENSUS":
        ...
        load_path_ratio1 = result_path / "attacker" / args.filter / "ModelNum" / str(args.model_number) / str(args.ratio1) 
        load_path_ratio2 = result_path / "attacker" / args.filter / "ModelNum" / str(args.model_number) / str(args.ratio2)

        test_load_path_ratio1 = result_path / "victim" / args.filter / "ModelNum" / str(args.model_number) / str(args.ratio1)
        test_load_path_ratio2 = result_path / "victim" / args.filter / "ModelNum" / str(args.model_number) / str(args.ratio2)
    ...
    model_list_ratio1, model_list_ratio2, test_model_list_ratio1, test_model_list_ratio2 = utils.get_model_list_capacity_exp(args, load_path_ratio1, load_path_ratio2, test_load_path_ratio1, test_load_path_ratio2, X_test_adv.shape[1])
...
def handle_args() -> argparse.Namespace:
    ...
    parser.add_argument('--ratio2', type=float, default=0.1,help='what ratio of the new sampled dataset')
