...
def main(args: argparse.Namespace, log: logging.Logger) -> None:
    ...
    if args.dataset == "CENSUS":
        ...
        if args.num_features == 42:
            ...
            load_path_ratio1 = result_path / "attacker" / args.filter / str(args.num_features) / str(args.ratio1)
            load_path_ratio2 = result_path / "attacker" / args.filter / str(args.num_features) / str(args.ratio2)

            test_load_path_ratio1 = result_path / "victim" / args.filter / str(args.num_features) / str(args.ratio1)
            test_load_path_ratio2 = result_path / "victim" / args.filter / str(args.num_features) / str(args.ratio2)
    ...
    print("Original Number of Features", X_test_adv.shape[1])
    from sklearn.feature_selection import SelectKBest
    from sklearn.feature_selection import f_classif
    X_test_adv = SelectKBest(f_classif, k=args.num_features).fit_transform(X_test_adv, y_test_adv)
    print("Number of Features", X_test_adv.shape[1])
