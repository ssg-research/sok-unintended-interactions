    ...
    if args.dataset == "CENSUS":
        ...
        for i in range(args.num):
            ...
            if args.model_number == 1:
                model = models.Model1(X_train.shape[1]).to(args.device)
                print(models.count_parameters(model))
                ...
                model = models.Model2(X_train.shape[1]).to(args.device)
                print(models.count_parameters(model))
                model, trainacc, testacc = utils.train(args.epochs, model, trainloader, testloader, torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.decay), args, log)

            elif args.model_number == 3:
                model = models.Model3(X_train.shape[1]).to(args.device)
                print(models.count_parameters(model))
                model, trainacc, testacc = utils.train(args.epochs, model, trainloader, testloader, torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.decay), args, log)

            else:
                model = models.Model4(X_train.shape[1]).to(args.device)
                print(models.count_parameters(model))
                model, trainacc, testacc = utils.train(args.epochs, model, trainloader, testloader, torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.decay), args, log)

            save_path = result_path / args.split / args.filter / "ModelNum" / str(args.model_number) / str(args.ratio)
