...
def main(args: argparse.Namespace, log: logging.Logger) -> None:
    ...
    if args.modelnum == 1:
        ...
        clf = models.Classifier2(n_features=n_features)
    elif args.modelnum == 3:
        clf = models.Classifier3(n_features=n_features)
    elif args.modelnum == 4:
        clf = models.Classifier4(n_features=n_features)
    print(models.count_parameters(clf))    
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, filename="modelsize_inversion_attack.log", filemode="w")
    log: logging.Logger = logging.getLogger("ModelSizeInversionAttack")
    args = handle_args()
    main(args, log)